const snippets: { [trigger: string]: string } = {
  ...
  function handleContentEditableInput(event: KeyboardEvent, target: HTMLElement) {
  ...
  function whenKeydown(event: KeyboardEvent): void {
  ...
  function attachListenersToEditableFields(root: Document = document): void {
    const editableElements = root.querySelectorAll<HTMLInputElement | HTMLTextAreaElement | HTMLElement>(
      'input, textarea, [contenteditable="true"]'
    );
    editableElements.forEach((element) => {
      element.addEventListener("keydown", (evt: Event) => {
        whenKeydown(evt as KeyboardEvent);
      });
    });
  ...
  function attachListenersToIframe(iframe: HTMLIFrameElement): void {
    try {
  ...
  function attachListenersToAllIframes(parentDocument: Document = document): void {
  ...
  function monitorDynamicElements(): void {
